...
export class DatabaseMaintenanceService {
  ...
  static async healthCheck(): Promise<{
    ...
    try {
      ...
      const { count: recentSignups, error: recentSignupsError } =
        await supabaseAdmin
          .from('beta_signups')
          .select('id', { count: 'exact' })
      ...
      if (recentSignupsError) {
        issues.push(
          `Cannot check recent signups: ${recentSignupsError.message}`
        )
      }

      // Check recent email events (last 24h)
      const { count: recentEmailEvents, error: recentEmailError } =
        await supabaseAdmin
          .from('email_events')
          .select('id', { count: 'exact' })
      ...
      if (recentEmailError) {
        issues.push(
          `Cannot check recent email events: ${recentEmailError.message}`
        )
      }

      // Check for users with missing preferences
      const { count: usersWithoutPrefs, error: prefsError } =
        await supabaseAdmin
          .from('beta_signups')
          .select(
            `
      ...
      if (prefsError) {
        ...
        issues.push(
          `${usersWithoutPrefs} users missing preferences (${Math.round(((usersWithoutPrefs || 0) / (totalSignups || 1)) * 100)}%)`
        )
      }
    ...
    try {
      ...
      const userStats = {
        ...
        avg_engagement_score:
          users.reduce((sum, u) => sum + (u.engagement_score || 0), 0) /
          users.length,
      }
      ...
      const uniqueVisitors = new Set(
        analytics.map((a) => a.visitor_id).filter(Boolean)
      ).size
