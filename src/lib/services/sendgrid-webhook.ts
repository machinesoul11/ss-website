...
interface SendGridEvent {
  email: string
  timestamp: number
  event: 'delivered' | 'opened' | 'clicked' | 'bounce' | 'dropped' | 'spamreport' | 'unsubscribe'
  sg_event_id: string
...
export class SendGridWebhookService {
  ...
  static async processWebhookEvents(
    ...
    try {
      ...
      if (publicKey) {
        ...
        const isValid = this.verifyWebhookSignature(publicKey, payload, signature, timestamp)
        if (!isValid) {
  ...
  static async getDeliverabilityMetrics(days = 30): Promise<{
    ...
    try {
      ...
      if (deliveredRate < 95) {
        issues.push(`Low delivery rate: ${deliveredRate.toFixed(1)}% (target: >95%)`)
      }
      ...
      if (spamRate > 0.1) {
        issues.push(`High spam complaint rate: ${spamRate.toFixed(2)}% (target: <0.1%)`)
      }
