...
export function useCombinedAnalytics() {
  ...
  const trackEvent = useCallback(async (event: CombinedAnalyticsEvent) => {
    const { name, properties = {}, plausibleProps, skipPlausible = false, skipCustom = false } = event
  ...
  const trackConversion = useCallback(async (goalName: string, value: number = 1, metadata: Record<string, any> = {}) => {
    try {
      // Track to custom analytics
  ...
  const trackBetaSignup = useCallback(async (formData: Record<string, any>, source?: string) => {
    try {
      // Track to custom analytics
      await trackEvent({
        name: 'beta_signup',
  ...
  const trackFormSubmission = useCallback(async (formName: string, success: boolean = true, data?: Record<string, any>) => {
    try {
      // Track to custom analytics
      await trackEvent({
  ...
  const trackCTAClick = useCallback(async (ctaText: string, position: string, page?: string) => {
    try {
      // Track to custom analytics
      await trackEvent({
        name: 'cta_click',
  ...
  const trackScrollDepth = useCallback(async (percentage: number, page?: string) => {
    try {
      // Track to custom analytics
      await trackEvent({
        name: 'scroll_depth',
  ...
  const trackTimeOnPage = useCallback(async (seconds: number, page?: string) => {
    try {
      // Track to custom analytics
      await trackEvent({
        name: 'time_on_page',
 ...
 */
function convertPropsToPlausible(properties: Record<string, any>): Record<string, string | number | boolean> {
