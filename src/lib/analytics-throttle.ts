...
import { shouldTrackAnalytics, analyticsDebug, analyticsConfig } from './analytics-config'
...
export function createThrottledPlausibleTracker() {
    ...
    const throttledTrack = globalThrottle.throttle(
        ...
        (eventName: string, data: any) => {
            if (typeof window !== 'undefined' && window.plausible && typeof window.plausible === 'function') {
    ...
    return {
        trackEvent: (eventName: string, data?: any) => {
            ...
            }, undefined, `plausible_${eventName}`)
        }
    }
}
...
export function createFormInteractionThrottler() {
    ...
    return {
        throttleFormEvent: (formId: string, fieldName: string, action: string, fn: () => void) => {
            ...
            const delay = delays[action as keyof typeof delays] || config.throttleDelay

            if (!throttler.rateLimit(key, {
                delay,
                maxCalls: config.maxFormEventsPerMinute || (action === 'change' ? 3 : 8),
...
export function createScrollTrackingThrottler() {
    ...
    return {
        trackScrollMilestone: (page: string, percentage: number, fn: () => void) => {
            ...
            if (!globalThrottle.rateLimit('scroll_tracking', {
                delay: config.throttleDelay,
