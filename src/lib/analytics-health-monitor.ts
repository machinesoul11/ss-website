...
class AnalyticsHealthMonitor {
    ...
    performHealthCheck(): boolean {
        ...
        if (now - this.metrics.lastEventTime > 300000) { // 5 minutes
            this.errors = []
            this.warnings = []
        }
    ...
    return new Promise((resolve) => {
        try {
            ...
            if (!healthMonitor.performHealthCheck()) {
                healthMonitor.addWarning(`Skipping ${operationName} due to health check failure`)
                resolve()
                return
            }
            ...
            result
                ...
                .catch((error) => {
                    healthMonitor.recordFailure(operationName, error.message || String(error))
                    resolve() // Don't reject - just log and continue
                })

        } catch (error) {
            healthMonitor.recordFailure(operationName, error instanceof Error ? error.message : String(error))
            resolve() // Don't reject - just log and continue
        }
    })
}
...
export function createProductionAnalytics() {
    return {
        ...
        trackCustom: (operation: () => void | Promise<void>, operationName: string) => {
