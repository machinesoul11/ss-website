...
export function PerformanceMonitoringDashboard() {
    ...
    try {
      ...
      if (errorResponse.ok) {
        ...
        const systemAlerts = recentErrors
          .filter((error: any) => error.metadata?.error?.severity === 'critical')
          .slice(0, 10)
      ...
      reportError('Failed to load dashboard data', { error: error instanceof Error ? error.message : String(error) })
      setIsLoading(false)
    }
  }
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
            <div className="text-center p-4 bg-gray-50 rounded-lg">
              <div className="text-2xl font-bold text-gray-900">
                {performanceData.lcp ? formatDuration(performanceData.lcp) : 'N/A'}
              </div>
            ...
            <div className="text-center p-4 bg-gray-50 rounded-lg">
              <div className="text-2xl font-bold text-gray-900">
                {performanceData.fid ? formatDuration(performanceData.fid) : 'N/A'}
              </div>
            ...
            <div className="text-center p-4 bg-gray-50 rounded-lg">
              <div className="text-2xl font-bold text-gray-900">
                {performanceData.ttfb ? formatDuration(performanceData.ttfb) : 'N/A'}
              </div>
              <div className="text-sm text-gray-600 mt-1">TTFB</div>
              <div className="text-xs text-gray-500 mt-1">Time to First Byte</div>
            </div>
            
            <div className={`text-center p-4 rounded-lg border ${getPerformanceScoreColor(performanceData.performanceScore)}`}>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-between items-center p-3 bg-gray-50 rounded">
                ...
                <span className={`text-lg font-semibold ${apiMetrics.errorRate > 5 ? 'text-red-600' : 'text-green-600'}`}>
                  {apiMetrics.errorRate.toFixed(1)}%
                </span>
              </div>
              
              <div className="flex justify-between items-center p-3 bg-gray-50 rounded">
                <span className="text-sm font-medium text-gray-700">Slow Requests (&gt;3s)</span>
                <span className={`text-lg font-semibold ${apiMetrics.slowRequests > 0 ? 'text-yellow-600' : 'text-green-600'}`}>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center p-3 bg-red-50 rounded border border-red-200">
                <span className="text-sm font-medium text-red-700">Critical Errors</span>
                <span className="text-lg font-semibold text-red-900">{errorStats.critical}</span>
              </div>
              ...
              <div className="flex justify-between items-center p-3 bg-orange-50 rounded border border-orange-200">
                ...
                <span className="text-lg font-semibold text-orange-900">{errorStats.high}</span>
              </div>
              ...
              <div className="flex justify-between items-center p-3 bg-yellow-50 rounded border border-yellow-200">
                ...
                <span className="text-lg font-semibold text-yellow-900">{errorStats.medium}</span>
              </div>
              
              <div className="flex justify-between items-center p-3 bg-blue-50 rounded border border-blue-200">
                <span className="text-sm font-medium text-blue-700">Low Priority</span>
                <span className="text-lg font-semibold text-blue-900">{errorStats.low}</span>
              </div>
              ...
              <div className="flex justify-between items-center p-3 bg-gray-50 rounded border border-gray-200">
                ...
                <span className="text-lg font-semibold text-gray-900">{errorStats.total}</span>
              </div>
            </div>
          </div>
        )}
      ...
      {alerts.length > 0 && (
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <h3 className="text-lg font-medium text-gray-900 mb-4">Recent Critical Alerts</h3>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="flex items-center space-x-3 p-3 bg-green-50 rounded-lg border border-green-200">
            ...
            <div>
              <div className="text-sm font-medium text-green-900">Performance Monitoring</div>
          ...
          <div className="flex items-center space-x-3 p-3 bg-green-50 rounded-lg border border-green-200">
            <div className="w-3 h-3 bg-green-500 rounded-full"></div>
            <div>
              <div className="text-sm font-medium text-green-900">Error Tracking</div>
              <div className="text-xs text-green-700">Active</div>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-3 p-3 bg-green-50 rounded-lg border border-green-200">
            <div className="w-3 h-3 bg-green-500 rounded-full"></div>
            <div>
              <div className="text-sm font-medium text-green-900">API Monitoring</div>
              <div className="text-xs text-green-700">Active</div>
            </div>
          </div>
        </div>
