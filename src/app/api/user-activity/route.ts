...
export async function GET(request: NextRequest) {
  try {
    ...
    const [user, emailEvents, feedback, analytics] = await Promise.all([
      supabaseAdmin.from('beta_signups').select('*').eq('id', userId).single(),
    ...
    const emailOpens =
      emailEvents.data?.filter((e) => e.event_type === 'opened').length || 0
    const emailClicks =
      emailEvents.data?.filter((e) => e.event_type === 'clicked').length || 0
    const pageViews = analytics.data?.length || 0
    const avgSessionTime =
      analytics.data?.reduce((sum, a) => sum + (a.session_duration || 0), 0) /
