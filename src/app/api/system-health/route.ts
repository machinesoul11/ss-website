...
export async function GET(request: NextRequest) {
  try {
    ...
    const deliveryRate = emailSent.count && emailSent.count > 0 
    ...
    const healthData = {
      ...
      email: {
        status: deliveryRate > 95 ? 'healthy' : deliveryRate > 90 ? 'warning' : 'error',
      ...
      api: {
        status: apiResponseTime < 500 ? 'healthy' : apiResponseTime < 1000 ? 'warning' : 'error',
        responseTime: apiResponseTime,
        errorRate: ((systemErrors.count || 0) / Math.max(totalUsers.count || 1, 1)) * 100,
        uptime: 99.8
      },
      metrics: {
