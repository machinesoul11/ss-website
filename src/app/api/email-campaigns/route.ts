    ...
    switch (validatedData.type) {
      case 'welcome':
        ...
        return NextResponse.json(
          {
            error:
              'Welcome emails should be sent individually via POST /api/beta-signup',
          },
          { status: 400 }
        )
      ...
      case 'development_update':
        ...
        const segmentFilter =
          validatedData.segmentFilter === 'high_engagement'
...
export async function GET(request: NextRequest) {
  try {
    ...
    const { segments: engagementSegments } =
      await SegmentationService.segmentByEngagement()
    const { segments: toolSegments } =
      await SegmentationService.segmentByTools()
    const { segments: teamSegments } =
      await SegmentationService.segmentByTeamSize()
    ...
    return NextResponse.json({
      ...
      data: {
        segments: {
          highEngagement:
            engagementSegments.find((s) => s.id === 'high-engagement')?.count ||
            0,
          mediumEngagement:
            engagementSegments.find((s) => s.id === 'medium-engagement')
              ?.count || 0,
          lowEngagement:
            engagementSegments.find((s) => s.id === 'low-engagement')?.count ||
            0,
          valeUsers:
            toolSegments.find((s) => s.id === 'vale-users')?.count || 0,
          grammarlyUsers:
            toolSegments.find((s) => s.id === 'grammarly-users')?.count || 0,
          individualContributors:
            teamSegments.find((s) => s.id === 'individual')?.count || 0,
          smallTeams:
            teamSegments.find((s) => s.id === 'small-team')?.count || 0,
        },
        recentCampaigns,
        emailStats,
      },
