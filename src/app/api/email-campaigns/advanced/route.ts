...
const abTestSchema = z.object({
  campaign_type: z.enum(['development_update', 'newsletter', 'feedback_request']),
...
const earlyAccessSchema = z.object({
  ...
  exclusive_features: z.array(z.string()).min(1, 'At least one feature required'),
...
export async function GET(request: NextRequest) {
  try {
    ...
    const timeframe = url.searchParams.get('timeframe') as 'week' | 'month' | 'quarter' || 'month'

    switch (type) {
      case 'engagement':
        const analytics = await EmailOptimizationService.getEngagementAnalytics(timeframe)
        return NextResponse.json({
          success: true,
      ...
      case 'send_times':
        const sendTimeAnalysis = await EmailOptimizationService.analyzeOptimalSendTimes()
        return NextResponse.json({
          success: true,
...
async function handleOptimizationAnalysis(body: unknown) {
  ...
  if (engagementAnalytics.analytics) {
    const { overall_rates, by_day_of_week, by_hour } = engagementAnalytics.analytics
    
    if (overall_rates.open_rate < 20) {
      recommendations.push({
        type: 'open_rate',
        priority: 'high',
        message: 'Overall open rate is below industry average (20-25%). Consider improving subject lines.',
    ...
    if (overall_rates.click_rate < 3) {
      recommendations.push({
        type: 'click_rate',
        priority: 'medium',
        message: 'Click rate is below average. Consider improving email content and CTAs.',
    ...
    const bestHour = by_hour.reduce((best, current) => 
      current.send_count > 5 && current.open_rate > best.open_rate ? current : best
    )
