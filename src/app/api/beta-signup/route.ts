...
import { BetaSignupService, EmailCampaignService, AnalyticsService } from '@/lib/services'
...
const betaSignupSchema = z.object({
  ...
  currentTools: z.array(z.string()).min(1, 'At least one tool must be selected'),
  documentationPlatforms: z.array(z.string()).min(1, 'At least one platform must be selected'),
  painPoints: z.string().min(10, 'Please describe your main pain points (at least 10 characters)'),
  teamSize: z.enum(['individual', 'small_team', 'medium_team', 'large_team', 'enterprise']).optional(),
  useCaseDescription: z.string().min(10, 'Use case description must be at least 10 characters'),
  ...
  try {
    ...
    if (process.env.SENDGRID_API_KEY && validatedData.marketingOptIn) {
      try {
        ...
        const welcomeResult = await EmailCampaignService.sendWelcomeEmail(insertedUser)
