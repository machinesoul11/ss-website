...
export async function POST(request: NextRequest) {
  try {
    ...
    if (publicKey) {
      ...
      const isValid = verifyWebhookSignature(publicKey, body, signature, timestamp)
      if (!isValid) {
    ...
    const { SendGridWebhookService } = await import('@/lib/services/sendgrid-webhook')
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    await SendGridWebhookService.processWebhookEvents(mockReq as any, mockRes as any)
